...
使用 Python Agent SDK 开始构建能够自主工作的 AI 代理
...
1. **`query`**：创建代理循环的主入口点。它返回一个异步迭代器，所以你使用 `async for` 来流式传输 Claude 工作时的消息。请参阅 [Python SDK 参考](/docs/zh-CN/agent-sdk/python#query)中的完整 API。

2. **`prompt`**：你想让 Claude 做什么。Claude 根据任务确定要使用哪些工具。

3. **`options`**：代理的配置。此示例使用 `allowedTools` 将 Claude 限制为 `Read`、`Edit` 和 `Glob`，并使用 `permissionMode: "acceptEdits"` 自动批准文件更改。其他选项包括 `systemPrompt`、`mcpServers` 等。请参阅 [Python SDK 参考](/docs/zh-CN/agent-sdk/python#claudeagentoptions)中的所有选项。
